...
| # | Misconfiguration | Where | ATT&CK technique(s) | Status | Detection (Phase 4) |
|---|---|---|---|---|---|
| 1 | Service account with an SPN set and a weak/crackable password (Kerberoastable) | `svc-sql` on `dc01` | [T1558.003 — Kerberoasting](https://attack.mitre.org/techniques/T1558/003/) | Implemented in `config/dc/tasks/misconfigs.yml` — not run-tested | [`detections/sigma/kerberoasting.yml`](../detections/sigma/kerberoasting.yml) — passes `make detections-test` against fixtures |
| 2 | User account with Kerberos pre-authentication disabled (AS-REP roastable) | `svc-legacy` on `dc01` | [T1558.004 — AS-REP Roasting](https://attack.mitre.org/techniques/T1558/004/) | Implemented in `config/dc/tasks/misconfigs.yml` — not run-tested | [`detections/sigma/asrep_roasting.yml`](../detections/sigma/asrep_roasting.yml) — passes `make detections-test` against fixtures |
